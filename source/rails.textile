...
This command should create rails application in +todos+ directory. Now we need to add +bulk_api+ gem which will be used to provide integration with sproutcore easier. The main purpose of this gem is to provide API that can be used in conjunction with +SC.BulkDataSource+. It also helps with running sproutcore application - instead of running sproutcore server separately and setup proxy between 2 applications, you can run directly in rails.
...
# Now we can change branch to part_two, to start from second part of tutorial
git checkout -b part_two origin/part_two

# We also want to clone bulk_data_source into frameworks, it will be needed later:
...
Try running +todos+ application. It should run just fine, but it uses fixtures as a data source, so you will loos all your changes after page refresh. Let's change that!
...
<javscript filename="app/sproutcore/apps/todos/todos.js">
...
The last thing that you need to change is +Buildfile+ we need to require +SC.BulkDataSource+ in order to use it. Open Buildfile and change config line to the following:

<ruby filename="Buildfile">
config :all, :required => ["sproutcore/core_foundation", "bulk_data_source"], :theme => "sproutcore/empty_theme"
</ruby>
...
In the next sections of that guide I will add some UI elements that needs additional styling. As I don't want to take time to tweak CSS file, please just replace the entire CSS file with this file: https://github.com/sproutcore/Todos-Example/blob/bulk_api/apps/todos/resources/stylesheets/todos.css
...
<javascript filename="app/sproutcore/apps/todos/views/sign_in_overlay">
...
I set +sign_in+ template to use with that view, so now we need to add this template:

<html filename="app/sproutcore/apps/todos/resources/templates/sign_in.handlebars">
// TODO: fix SC.TextField to allow set placeholder and type and use that instead Todos.TextField
// TODO: figure out the best way to follow web standards, that is: when user
         presses enter, it should fire SC.Button
...
Here we have 2 text fields created using +SC.TextField+. They're bound to +Todos.userSessionController+, so when value of any of the todos is changed, corresponding attribute will be set in controller. +SC.Button+ is set to run +signIn()+ function on +Todos.userSessionController+.
